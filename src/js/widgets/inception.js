const template = {
  ...
  item: function(value){
    return `<li data-facet-value="${value}">${value}</li>`;
  }
}
...
function updateList ($list, attribute, results){
  ...
  const list = facets.map(function(facet){
    return template.item(facet.name);
  }).join('');
...
function updateListSearchOnce(attribute, $list, response) {
  ...
  const list = results.hits.map(function(hit){
    const value = (hit._highlightResult[attribute] && hit._highlightResult[attribute].value)|| hit[attribute];
